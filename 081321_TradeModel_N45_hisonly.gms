...
PARAMETER   ttflow(c,source,market)   NET import in EACH MARKET per metric ton;
            ttflow('corn',source,'china')=1900000;
            ttflow('sb',source,'china')=73700000;
            ttflow('wt',source,'china')=700000;
            ttflow('srg',source,'china')=4975000;
;
...
TABLE   tcaa(c,source,market)   trans costs per metric ton based on no tc results
*20% of source market price
                  us         china        row
corn.us            0          28           28
corn.china        50           0           50
corn.row          39          39            0
...
Table subtax(c,source,market) SUBSIDIES AND OR TAXES ON OUTGOING SHIPMENTS
                 us     china   row
corn.us           0      18       0
corn.china        0     -70       0
corn.row          0      23       0
...
OUT(c,i,f)           Output for fertilizer use
OUT2(i)              edge of field N runoff county level
OUT3(i)              delivered N runoff county level
OUT4(i,c)            crop hectare by county
OUT5(j,c)            crop hectare by county omrb
OUT6(i)              fertilizer use level by county within MRB
OUT7(c)              endogenous crop price
OUT8(i,c)            crop production within the MRB
OUT9(j,c)            crop production outside the MRB
REVENUEW(i)          county level revenue for counties within the MRB
REVENUEO(j)          county level revenue for counties outside the MRB
...
*TCCALIBRATION(c,market) TRANSACTION COST EQUARION FOR CALIBRATION
...
*TCCALIBRATION(c,market)..SUM(source,SHIPMENTS(c,source,'china'))=E= SUM(source,ttflow(c,source,'china'));
...
NCONS..TOTNGULF=L=435670000*0.55;
...
WELFAREUS.L=SUM((k,c),LAMBDA.L(c,k)*ybar(k,c))-SUM(c,TOTCOST.L(c))-SUM((c,market),SHIPMENTS.L(c,'US',market)*(tcga(c,'US',market)+subtax(c,'US',market)));
Display WELFAREUS.L;
OUT4.L(i,c)=SUM((f,w),CROPHA.L(c,i,f,w));
OUT5.L(j,c)= CROPHAO.L(j,c);
OUT8.L(i,c)=SUM((f,w),CROPHA.L(c,i,f,w)*cropyieldirri(i,c,w,f));
OUT9.L(j,c)=CROPHAO.L(j,c)*cropyieldomrb(j,c)
$LIBInclude xldump OUT4.L "C:\YueluResearch\TradePaper\GAMScode_for_replication\trade_N45.xlsx" wcropha!a1
$LIBInclude xldump OUT5.L "C:\YueluResearch\TradePaper\GAMScode_for_replication\trade_N45.xlsx" ocropha!a1
$LIBInclude xldump OUT8.L "C:\YueluResearch\TradePaper\GAMScode_for_replication\trade_N45.xlsx" wcropprod!a1
$LIBInclude xldump OUT9.L "C:\YueluResearch\TradePaper\GAMScode_for_replication\trade_N45.xlsx" ocropprod!a1
